...
function TimelineInner() {
  ...
  const initialMode = (params.get("view") as Mode) ?? "gantt";
  const [mode, setMode] = useState<Mode>(initialMode);
  const [openEpicKey, setOpenEpicKey] = useState<string | null>(null);

  // Reflect mode to URL for shareability
  useEffect(() => {
  ...
  return (
    <div>
      <PageHeader
        ...
        lede="Four lenses on the same Epics. Gantt for overlap, Month for this-week density, Agenda for dates, Swimlane for capacity."
        actions={<ModeStrip mode={mode} onChange={setMode} />}
      />
...
function ModeStrip({ mode, onChange }: { mode: Mode; onChange: (m: Mode) => void }) {
...
function healthBgSoft(h: Epic["health"]) {
  return h === "on_track" ? "bg-ok-soft" : h === "at_risk" ? "bg-warn-soft" : h === "blocked" ? "bg-danger-soft" : "bg-neutral-soft";
}
...
function startOfWeek(d: Date) {
  const x = new Date(d);
  const day = x.getDay();
  x.setDate(x.getDate() - day); // Sunday-start grid
  x.setHours(0, 0, 0, 0);
  return x;
}
...
// ─── Gantt (existing, lightly trimmed) ───────────────────────────
...
function MonthView({
  ...
  const cells = Array.from({ length: 42 }, (_, i) => addDays(gridStart, i)); // 6 rows × 7 cols
...
function SwimlaneView({
  ...
  // Visible epics: those overlapping the quarter
  ...
  const groups: Group[] = useMemo(() => {
    ...
    if (group === "pod") {
      // Pod = the dominant pod among an epic's projects
          ...
          {groups.map((g) => (
            <div key={g.key} className="flex border-b border-rule-soft">
              ...
              <div className="relative flex-1 py-2" style={{ minHeight: 24 * Math.max(1, g.epics.length) + 16 }}>
                ...
                {/* Epic mini-bars stacked vertically */}
